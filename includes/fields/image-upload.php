...
class Image_Upload extends \WPForms_Field {
	...
	public function init() {
		...
		$this->name  = __( 'Image Upload', 'upload-fields-for-wpforms' );
	...
	public function html_field_value( $value, $field, $form_data, $type ) {
		...
		foreach ( $uploaded_files as $file ) { ?>
            <div style="display: flex; align-items: center; margin-bottom: 5px; padding: 3px; border: 1px solid #ddd;background-color: #FAFAFA;border-radius:3px;">
	...
	public function file_icon_html( $file ) {
		printf( '<span class="file-icon"><img src="%s" style="margin-right:5px;height:auto;width:30px;max-width:30px;vertical-align: middle;" width="30" height="30"  /></span>', esc_url( $file['value'] ) );
	}
	...
	public function render_uploader( $field, $form_data = null ) {
		...
		?>
        <div class="upwpforms-uploader image-upload"
             ...
             data-form-id="<?php echo esc_attr( ! empty( $form_data['id'] ) ? $form_data['id'] : '' ); ?>"
        >
            ...
            <div class="uploader-hint">
                <span class="max-files-label <?php echo esc_attr( $max_files < 2 ? 'hidden' : '' ); ?>"><?php printf( __( "Upload upto %s Images.", 'upload-fields-for-wpforms' ), '<span class="number">' . $max_files . '</span>' ); ?></span>
                <span class="max-size-label <?php echo esc_attr( empty( $max_size ) ? 'hidden' : '' ); ?>"><?php printf( __( "Max Image Size: %s", 'upload-fields-for-wpforms' ), '<span class="size">' . $max_size . '</span>' ); ?></span>
            </div>
        </div>
		<?php
	...
	public function field_options( $field ) {
		...
		$lbl = $this->field_element( 'label', $field, array(
			'slug'    => 'extensions',
			'value'   => esc_html__( 'Allowed Images', 'upload-fields-for-wpforms' ),
			'tooltip' => esc_html__( 'Enter the comma seperated extensions of images you would like to allow to upload.', 'upload-fields-for-wpforms' ),
		), false );

		$fld = $this->field_element( 'text', $field, array(
			'slug'  => 'extensions',
			'value' => ! empty( $field['extensions'] ) ? $field['extensions'] : 'jpg, jpeg, png, gif',
		), false );
