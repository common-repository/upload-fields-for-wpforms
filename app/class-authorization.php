...
class Authorization {
	...
	private $token_key = 'upwpforms_tokens';
	private $client;
	...
	public function refresh_token( $account ) {
		...
		try {
			...
			if ( $timestamps = wp_next_scheduled( 'upwpforms_lost_authorization_notice', [ 'account_id' => $account['id'] ] ) ) {
				wp_unschedule_event( $timestamps, 'upwpforms_lost_authorization_notice', [ 'account_id' => $account['id'] ] );
			...
			if ( ! wp_next_scheduled( 'upwpforms_lost_authorization_notice', [ 'account_id' => $account['id'] ] ) ) {
				wp_schedule_event( time(), 'daily', 'upwpforms_lost_authorization_notice', [ 'account_id' => $account['id'] ] );
